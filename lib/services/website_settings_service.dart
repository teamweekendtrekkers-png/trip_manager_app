...
class WebsiteSettingsService {
  ...
  WebsiteSettingsService({required this.settings, Dio? dio})
    : _dio =
          dio ??
          Dio(
            BaseOptions(
    ...
    try {
      ...
      if (!_isValidUpi(newUpiId)) {
        return UpdateResult(
          success: false,
          error:
              'Invalid UPI ID format. Expected: name@provider (e.g., 9538236581@ybl or business@icici)',
        );
      }
      ...
      return UpdateResult(success: false, error: _getErrorMessage(e));
    } catch (e) {
      return UpdateResult(success: false, error: 'Failed to update UPI: $e');
    }
  }
      ...
      try {
        ...
        if (maskedPattern.hasMatch(content)) {
          ...
          await _dio.put(
            ...
            data: {
              'message':
                  'Update masked UPI display in $file via Trip Manager App',
              'content': base64.encode(utf8.encode(updatedContent)),
              'sha': sha,
              'branch': settings.branch,
            },
    ...
    try {
      ...
      if (!_isValidWhatsApp(newNumber)) {
        return UpdateResult(
          success: false,
          error:
              'Invalid WhatsApp number format. Use format: 917019235581 (country code + number)',
        );
      }
      ...
      for (final file in htmlFiles) {
        try {
          final result = await _updateWhatsAppInFile(
            file,
            currentNumber,
            newNumber,
          );
      ...
      return UpdateResult(
        success: true,
        message:
            'WhatsApp updated in $updatedCount files (from $currentNumber to $newNumber)',
  ...
  Future<UpdateResult> updateBothSettings(
    String newUpiId,
    String newWhatsApp,
  ) async {
    final results = <String>[];
  ...
  Future<WebsiteSettings> getCurrentWebsiteSettings() async {
    ...
    try {
      ...
      final securityContent = utf8.decode(
        base64.decode(
    ...
    return WebsiteSettings(upiId: upiId, whatsAppNumber: whatsAppNumber);
  }
  ...
  Future<bool> _updateWhatsAppInFile(
    String filePath,
    String oldNumber,
    String newNumber,
  ) async {
...
class UpdateResult {
  ...
  UpdateResult({required this.success, this.message, this.error, this.details});
}
...
class WebsiteSettings {
  ...
  WebsiteSettings({this.upiId, this.whatsAppNumber});
}
